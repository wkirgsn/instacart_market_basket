...
def load_input_data():
    ...
    prior = pd.read_csv('data/order_products__prior.csv', dtype={'order_id':
                                                                np.int32,
    ...
    orders = pd.read_csv('data/orders.csv', dtype={'order_id': np.int32,
                                                  'user_id': np.int64,
...
if __name__ == '__main__':
    ...
    df = pd.concat([add_order_streak(grp) for _, grp in user_groups])
    e = datetime.now()
